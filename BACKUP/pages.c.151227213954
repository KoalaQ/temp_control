...
void page1(void){
 ...
 void page2(void){
   ...
   ft2=EEPROM_read(3,0);
   ft3=EEPROM_read(4,0);
 if(!(ft0==0 && ft1==0 && ft2==0 && ft3==0)){//ÅĞ¶ÏÊÇ·ñÓĞÊı¾İ£¬ÈÎºÎÒ»¸öÓĞÊı¾İ¾ÍÑ¯ÎÊÊÇ·ñ»Ö¸´
   //³õÊ¼»­Ãæ£¬»áÓĞ¶ÁÈ¡EEPROMµÄÅĞ¶ÏÄ¬ÈÏ²Ù×÷£¬±¾´ÎÖ»ÊÇ»­½çÃæ
   lcd_write_str(2,0,"ÊÇ·ñ»Ö¸´ÉÏ´Î¹¤×÷");
   lcd_write_str(2,2,"»Ö¸´");
   lcd_write_str(8,2,"È¡Ïû");
   ft3=EEPROM_read(0,0);//¸´ÓÃ¶ÁÈ¡Ê×Ñ¡Ïî
   if(ft3==1){//Ä¬ÈÏ»Ö¸´
     cur=0;
     Set_White(3,2,4);
   }else{
   ...
   Timerinit_0(0x00); //Ê¹ÓÃ¶¨Ê±Æ÷£¬¶¨Ê±Ò»¶ÎÊ±¼ä¸ü¸Ä±ê¼Ç£¬whileÖĞ¶ÁÈ¡Õâ¸ö±ê¼ÇÈ·¶¨ÊÇ·ñ½áÊøwhile
    while(1){
	if(waitflag==1){
	  //Ö´ĞĞÏà¹Ø²Ù×÷
		   Set_White_off(9,2,10);
			...
			if(cur==0){
			  lcd_clear();
			  lcd_write_str(5,2,"»Ö¸´ÖĞ. . .");
			  data_recover();//»Ö¸´Êı¾İ£¬¿ÉÒÔÌø×ªµ½½çÃæºó²Ù×÷¡£ÔİÊ±·ÅÔÚÕâ
			  delay_ms(200);
			}
		  pageNum=1;
		  t0_flag=1;
		  waitflag=32;
		  Timerinit_0(0x00);//¼ÆÊ±Æ÷¹¦ÄÜ¸Ä±ä£¬ÓÃ×÷¶ÁÈ¡ÎÂ¶ÈÊ¹ÓÃ.32´Î¼ÆÊıÍê³É0.5s
		  return;
	  }
      keyV=KeyScan_once();
	  switch(keyV){
	...
	switch(temps[pageParam[0]].flag){
		 ...
		 case 1:
		   ...
		   if(timecache>18){
			 ...
			 lcd_write_char_con(2,' ');
		   if(timecache & 0x01){
		       lcd_write_str_con(2,"ÉıÎÂ");
                timecache=temps[pageParam[0]].p_settemp;
		   }else{
		       lcd_write_str_con(2,"ºãÎÂ");
	  ...
	  if(keyV=='u'){
	    ...
	    switch(cur){
		  case 0:
			 ...
			 EEPROM_write(0,0,ddflag);//Ğ´Èëµ½EEPROM
			if(ddflag==0){
               lcd_write_char(3,0,0xA1);
               lcd_write_char_con(0,0xC1);
            }else{
...
void page9(void){//¸´ÓÃµÄ·½·¨
  if(pageParam[2]==0){
	 ...
	 //ÊÇ·ñĞèÒªÇå³ıÊı¾İ£¬¿¼ÂÇÖĞ
	 WDTCR|=(1<<WDCE)|(1<<WDE); /* ÖÃÎ» WDTOE ºÍ WDE(ÔÚÉèÖÃ¿´ÃÅ¹·¶¨Ê±Æ÷µÄÔ¤ÖÃ·ÖÆµÆ÷²ÎÊıÊ±ĞèÒª½«ÆäĞ´1)*/
     WDTCR=(1<<WDE)|(1<<WDP2)|(1<<WDP1); /*Éè¶¨ÖÜÆÚÎª1S*/
	 while(1){
	 }
